...
int main() {
  ...
  out << "algo,alphabet,text_size,pattern_size,avg_time,avg_oper\n";
        ...
        for (size_t text_size = 10000; text_size <= 100000; text_size *= 10) {
          for (size_t pattern_size = 100; pattern_size <= 3000;
            ...
            for (size_t text_i = 0; text_i < textpairs; ++text_i) {
              auto data =
                  generate_test(text_size, pattern_size, alphabet.second);
            ...
            out << algo.first << ' ' << alphabet.first << ' ' << text_size << ' ' << pattern_size << ' ' << avg_time << ' ' << avg_oper << '\n';
          }
        }
      }
    }
